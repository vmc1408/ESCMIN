...
// Intercepta e normaliza redirecionamentos do Supabase para a rota de redefinição de senha (`#/reset-password`)
(function handleSupabaseRedirects() {
  ...
  if (hash && (hash.includes('access_token=') || hash.includes('type=recovery') || hash.includes('error_description='))) {
    if (!hash.startsWith('#/reset-password')) {
      ...
      console.log('[Redirect Handler] Redirecionando hash do Supabase para a página de redefinição de senha:', cleanHash);
      window.location.hash = `#/reset-password?${cleanHash}`;
    }
  }
  ...
  if (search && (search.includes('code=') || search.includes('type=recovery'))) {
    ...
    console.log('[Redirect Handler] Redirecionando query params do Supabase para a página de redefinição de senha:', cleanSearch);
    
    // Limpa a query string do navegador para não sujar o histórico e redireciona via hash
    window.history.replaceState(null, '', window.location.pathname);
    window.location.hash = `#/reset-password?${cleanSearch}`;
  }
})();
