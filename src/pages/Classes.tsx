...
export function Classes() {
  ...
  return (
    <div className="h-[calc(100vh-6rem)] relative flex flex-col lg:flex-row gap-4 w-full p-4 overflow-hidden bg-slate-100/40">
      ...
      {selectedClass && (
        <div id="printable-class-record" className="hidden print:block text-black bg-white overflow-hidden font-sans leading-tight relative w-full h-[262mm] max-h-[265mm] mx-auto">
          <div className="w-full max-w-[180mm] mx-auto bg-white px-2 pt-1 pb-1 flex flex-col justify-between h-full">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 gap-2.5">
                  ...
                  <div className="border-b border-black/10 pb-1">
                    ...
                    <div className="text-[9pt] leading-relaxed text-justify whitespace-pre-line min-h-[28px]">
                      {(() => {
                        const rawObs = selectedClass.observations || '';
                        const cleanedObs = rawObs
                          .replace(/\[METADATA:.+?\]/gi, '')
                          .replace(/,?\s*"is_special"\s*:\s*(true|false)\}?/gi, '')
                          .replace(/,?\s*["']?is_special["']?\s*:\s*(true|false)\}?/gi, '')
                          .replace(/[\{\}]/g, '')
                          .replace(/^\s*,\s*/, '')
                          .trim();
                        return cleanedObs || 'Sem observações adicionais.';
                      })()}
                    </div>
                  </div>
                </div>
            ...
            {/* SIGNATURE BLOCK: Pushed down with generous vertical spacing */}
            <div className="mt-auto pt-14 pb-4 flex justify-between items-end px-2">
