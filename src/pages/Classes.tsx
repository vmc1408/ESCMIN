...
export function Classes() {
  ...
  const handleSelectCourseStartYearAndAcademicYear = React.useCallback((
    ...
    setFormData(prev => {
      ...
      // Keep user's entered name without auto-prefilling; user can use 'Sugerir Nome' button if desired
      const finalName = prev.name || '';
    ...
    try {
      ...
      if (startYrNum > parseInt(currentAcademicYear, 10)) {
        setHabilitatedMap(prev => {
          const yrKey = String(startYrNum);
          const currentList = prev[yrKey] || [];
          if (!currentList.includes(savedId)) {
            const updated = { ...prev, [yrKey]: [...currentList, savedId] };
            try {
              localStorage.setItem('academic_habilitated_classes_v1', JSON.stringify(updated));
            } catch (e) {}
            return updated;
          }
          return prev;
        ...
        <div className="flex-[1] flex flex-col overflow-hidden w-full bg-white">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-slate-50/30">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 px-4 text-center text-slate-400 gap-3">
                <Search size={32} className="text-slate-300" />
                <p className="text-[11px] font-bold uppercase tracking-wider text-slate-600">Nenhuma turma encontrada com os filtros atuais</p>
                {hasActiveFilters && (
                  <button
                    type="button"
                    onClick={handleClearFilters}
                    className="px-3 py-1.5 bg-slate-800 hover:bg-slate-900 text-white text-[10px] font-bold uppercase tracking-wider transition-colors cursor-pointer"
                  >
                    Exibir Todas as Turmas (Limpar Filtros)
                  </button>
                )}
              </div>
            ) : (
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900 tracking-tight leading-none uppercase">
                    {isEditing ? (selectedClass ? 'Edição da Turma' : 'Cadastro de Nova Turma') : formData.name}
                  </h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3.5 items-start pt-3 border-t border-blue-100/60">
                          ...
                          <div className="sm:col-span-1 lg:col-span-4 space-y-1.5">
                              ...
                              {isEditing && (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (autoName) {
                                      setFormData({ ...formData, name: autoName });
                                      setNotification({ type: 'success', message: `Nome padrão aplicado: ${autoName}` });
                                    } else {
                                      setNotification({ type: 'error', message: 'Selecione o Curso e o Ano para sugerir o nome padrão.' });
                                    }
                                  }}
                                  className="text-[9px] font-extrabold px-2 py-0.5 uppercase tracking-wider text-blue-900 bg-blue-100 hover:bg-blue-200 border border-blue-300 flex items-center gap-1 cursor-pointer transition-all shadow-2xs shrink-0"
                            ...
                            <input 
                              type="text"
                              disabled={!isEditing}
                              placeholder="EX: TEOLOGIA 2027, LATIM 2026..."
                              value={formData.name || ''}
                            ...
                            <p className="text-[9px] font-medium text-slate-400 italic ml-0.5 leading-tight truncate">
                              {isEditing ? "Nome oficial ou código identificador" : "Identificador oficial da turma."}
                            </p>
                          </div>
                          ...
                          <div className="sm:col-span-1 lg:col-span-3 space-y-1.5">
                            ...
                            <input 
                              ...
                              onKeyDown={handleKeyDown}
                              className="w-full px-3.5 py-2.5 bg-white border border-slate-300 text-xs font-bold text-slate-700 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 outline-none transition-all h-[42px] uppercase placeholder:text-slate-300"
                              tabIndex={2}
                            />
                          ...
                          <div className="sm:col-span-1 lg:col-span-2 space-y-1.5">
                                ...
                                if (schedDate) {
                                  return (
                                    <button
                                      ...
                                      onClick={() => {
                                        if (isEditing) {
                                          setFormData(prev => ({ ...prev, start_date: formatDateForDisplay(schedDate) }));
                                          setNotification({ type: 'success', message: `Data do cronograma inserida: ${formatDateForDisplay(schedDate)}` });
                                        }
                                      }}
                                      className="text-[9px] font-extrabold text-emerald-800 bg-emerald-100 hover:bg-emerald-200 border border-emerald-300 px-1.5 py-0.5 uppercase tracking-wider transition-colors flex items-center gap-1 cursor-pointer shadow-2xs shrink-0"
                                      title="Clique para preencher com a data oficial do cronograma acadêmico"
                                    >
                                      <CalendarCheck size={10} className="text-emerald-700" />
                                      <span>Sugerir</span>
                                    </button>
                                  );
                                }
                            ...
                            <div className="relative flex items-center">
                              <input 
                                ...
                                onKeyDown={handleKeyDown}
                                className="w-full pl-3 pr-8 py-2 bg-white border border-slate-300 text-xs font-bold text-slate-700 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 outline-none transition-all h-[42px] font-mono placeholder:text-slate-300"
                                tabIndex={3}
                              />
                              <input
                                type="date"
                                ...
                                onChange={(e) => {
                                  if (e.target.value) {
                                    ...
                                    setFormData({ ...formData, start_date: `${d}/${m}/${y}` });
                                  }
                                }}
                                className="absolute right-1 opacity-0 w-6 h-6 cursor-pointer z-10"
                          ...
                          <div className="sm:col-span-1 lg:col-span-3 space-y-1.5">
                            <div className="flex items-center justify-between ml-0.5 h-6">
                              <label className="text-[11px] font-extrabold text-blue-950 uppercase tracking-widest flex items-center gap-1.5">
                                <Users size={13} className="text-blue-900 shrink-0" />
                                Alunos Ativos
                              </label>
                              <span className="text-[9px] font-bold text-blue-700 uppercase tracking-wider">
                                Enturmação
                            ...
                            {selectedClass ? (
                              <button
                                type="button"
                                onClick={() => handleOpenStudentsModal()}
                                className="w-full flex items-center justify-between px-3 bg-white hover:bg-blue-50/70 border border-slate-300 hover:border-blue-500 transition-all h-[42px] text-left cursor-pointer group shadow-2xs rounded-none"
                                title="Clique para gerenciar ou visualizar os alunos matriculados nesta turma"
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <div className="w-6 h-6 bg-blue-900 text-white flex items-center justify-center font-bold text-xs shrink-0 group-hover:bg-blue-950 transition-colors shadow-2xs">
                                    <Users size={12} />
                                  </div>
                                  <div className="min-w-0">
                                    <span className="text-xs font-black text-slate-900 uppercase block leading-none truncate">
                                      {selectedClassStudentCount !== null ? `${selectedClassStudentCount} Aluno(s)` : '0 Alunos'}
                                    </span>
                                ...
                                <span className="text-[9px] font-black text-blue-800 bg-blue-100 group-hover:bg-blue-900 group-hover:text-white px-2 py-1 uppercase tracking-wider transition-all border border-blue-200 shrink-0 flex items-center gap-1">
                                  <Eye size={10} />
                                  Ver
                                </span>
                              </button>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  setNotification({
                                    type: 'error',
                                    message: 'Salve a nova turma primeiro no botão "Salvar Cadastro" para poder matricular e gerenciar alunos.'
                                  });
                                }}
                                className="w-full flex items-center justify-between px-3 bg-slate-50 hover:bg-slate-100 border border-dashed border-slate-300 transition-all h-[42px] text-left cursor-pointer group shadow-2xs rounded-none"
                                title="Disponível após salvar o cadastro da nova turma"
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <div className="w-6 h-6 bg-slate-200 text-slate-500 flex items-center justify-center font-bold text-xs shrink-0">
                                    <Users size={12} />
                                  </div>
                                  <div className="min-w-0">
                                    <span className="text-xs font-bold text-slate-500 uppercase block leading-none truncate">
                                      0 Alunos
                                    </span>
                                ...
                                <span className="text-[8px] font-extrabold text-slate-500 bg-slate-200/80 px-1.5 py-0.5 uppercase tracking-wider shrink-0 border border-slate-300">
                                  Após Salvar
                                </span>
                              </button>
                            )}
                            <p className="text-[9px] font-medium text-slate-400 italic ml-0.5 leading-tight truncate">
                              {selectedClass ? "Visualizar lista de estudantes" : "Disponível após salvar cadastro"}
                            </p>
                          </div>
                        </div>
                      </div>
