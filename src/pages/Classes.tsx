...
export function Classes() {
  ...
  return (
    <div className="h-[calc(100vh-6rem)] relative flex flex-col lg:flex-row gap-4 w-full p-4 overflow-hidden bg-slate-100/40">
      ...
      {selectedClass && (
        <div id="printable-class-record" className="hidden print:block text-black bg-white overflow-hidden font-sans leading-tight relative w-full h-[265mm] max-h-[268mm] mx-auto">
          <div className="w-full max-w-[185mm] mx-auto bg-white px-3 pt-2 pb-2 flex flex-col justify-between h-full">
            {/* TOP BLOCK: Header + Document Title + Class Data */}
            <div>
              {/* Institutional Header */}
              <div className="flex items-center gap-4 mb-2 pb-1.5 border-b-2 border-black">
                <div className="flex-shrink-0 w-16 h-16 flex items-center justify-center">
                  {inst?.logo_url ? (
                    <img src={inst.logo_url} className="w-full h-full object-contain max-h-16" referrerPolicy="no-referrer" alt="Logo" />
                  ) : (
                    <div className="w-full h-full border border-slate-300 border-dashed flex flex-col items-center justify-center text-[7pt] text-slate-300 font-bold uppercase">
                ...
                <div className="flex-1 flex flex-col">
                  <p className="text-[9.5pt] font-semibold tracking-widest text-slate-800 leading-tight">DIOCESE DE GUARULHOS</p>
                  <h1 className="text-[15.5pt] font-bold uppercase tracking-tight text-black leading-tight my-0.5">
                    {inst?.name || 'ESCOLA DIOCESANA DE MINISTÉRIOS'}
                  </h1>
                  <p className="text-[10pt] font-bold text-slate-700 tracking-wide mt-0.5 uppercase">
              ...
              {/* Document Title */}
              <div className="bg-black text-white py-1 px-3 mb-3 flex justify-between items-center">
                <h2 className="text-[12pt] font-bold uppercase tracking-widest">FICHA DA TURMA</h2>
                <span className="text-[9.5pt] font-bold">Turma: {selectedClass.code}</span>
              </div>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 gap-2.5">
                  <div className="border-b border-black/10 pb-1">
                    <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Nome do Curso / Turma</p>
                    <p className="text-[11pt] font-bold uppercase text-[#00174b]">{selectedClass.name}</p>
                  </div>
                  
                  <div className="grid grid-cols-3 gap-2.5">
                    <div className="border-b border-black/10 pb-1">
                      <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Ano Letivo</p>
                      <p className="text-[10pt] font-bold">{selectedClass.year || '---'}</p>
                    </div>
                    <div className="border-b border-black/10 pb-1">
                      <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Semestre</p>
                      <p className="text-[10pt] font-bold uppercase">{selectedClass.semester || '---'}</p>
                    </div>
                    <div className="border-b border-black/10 pb-1">
                      <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Período</p>
                      <p className="text-[10pt] font-bold uppercase">{selectedClass.period || '---'}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-2.5">
                    <div className="border-b border-black/10 pb-1">
                      <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Sala</p>
                      <p className="text-[10pt] font-bold uppercase">{selectedClass.room || '---'}</p>
                    </div>
                    <div className="border-b border-black/10 pb-1">
                      <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Duração</p>
                      <p className="text-[10pt] font-bold uppercase">Início em: {selectedClass.start_date || '---'}</p>
                    </div>
                  </div>

                  <div className="border-b border-black/10 pb-1">
                    <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Dias da Semana</p>
                    <p className="text-[10pt] font-bold uppercase">{(selectedClass.days_of_week || []).join(', ') || 'Não definidos'}</p>
                  </div>

                  <div className="border-b border-black/10 pb-1">
                    <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Disciplinas Vinculadas (Matriz Curricular)</p>
                    <div className="space-y-1.5 mt-1">
                      {(() => {
                        ...
                        if (classSubs.length === 0) {
                          return <p className="text-[9pt] text-slate-400 italic">Nenhuma disciplina vinculada.</p>;
                        }

                        return (
                          <div className="grid grid-cols-2 gap-2.5">
                            <div>
                              <p className="text-[7.5pt] font-bold text-blue-800 uppercase tracking-wider mb-0.5">1º Semestre:</p>
                              {printSem1.length > 0 ? (
                                printSem1.map(s => (
                                  <p key={s.id} className="text-[8.5pt] font-bold text-[#00174b] uppercase flex items-center gap-1.5 ml-1">
                                    <span className="w-1.5 h-1.5 rounded-full bg-blue-600"></span>
                                    [{s.code}] {s.name}
                                  </p>
                                ))
                              ) : (
                                <p className="text-[7.5pt] text-slate-400 italic ml-1">Nenhuma</p>
                              )}
                            </div>
                            <div>
                              <p className="text-[7.5pt] font-bold text-emerald-800 uppercase tracking-wider mb-0.5">2º Semestre:</p>
                              {printSem2.length > 0 ? (
                                printSem2.map(s => (
                                  <p key={s.id} className="text-[8.5pt] font-bold text-[#00174b] uppercase flex items-center gap-1.5 ml-1">
                            ...
                            {printOther.length > 0 && (
                              <div className="col-span-2">
                                <p className="text-[7.5pt] font-bold text-slate-700 uppercase tracking-wider mb-0.5">Outras Disciplinas:</p>
                                {printOther.map(s => (
                                  <p key={s.id} className="text-[8.5pt] font-bold text-[#00174b] uppercase flex items-center gap-1.5 ml-1">
                  ...
                  <div className="border-b border-black/10 pb-1">
                    <p className="text-[7pt] font-bold text-slate-400 uppercase mb-0.5">Observações da Turma</p>
                    <div className="text-[9pt] leading-relaxed text-justify whitespace-pre-line min-h-[24px]">
                      {(() => {
            ...
            {/* SIGNATURE BLOCK: Pushed down with generous vertical spacing (Yellow area requirement) */}
            <div className="mt-auto pt-24 pb-4 flex justify-between items-end px-2">
              <div className="space-y-0.5">
                <p className="text-[9.5pt] font-bold text-slate-800">
                  Guarulhos, {new Date().toLocaleDateString('pt-BR')}
                </p>
                <p className="text-[7.5pt] text-slate-400 font-medium">Local e Data</p>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-[75mm] border-t-2 border-black mb-1"></div>
                <p className="text-[9.5pt] font-bold uppercase tracking-widest text-[#00174b]">Assinatura da Secretaria</p>
                <p className="text-[7pt] text-slate-400 font-bold mt-0.5 tracking-tight">Escola Diocesana de Ministérios - ESMIN</p>
              </div>
            </div>

            {/* Institutional Footer (Green area requirement) */}
            <div className="border-t-2 border-black pt-1.5 pb-0.5 flex justify-between items-start text-[7.5pt] font-bold text-black uppercase tracking-tight shrink-0">
              <div className="flex-1 space-y-0.5">
                <p className="leading-none text-[8pt]">
                  {inst?.address || 'AV. VENUS, 195 - ITAPEGICA - GUARULHOS/SP'}
                </p>
                <p className="leading-none text-[8pt]">
                  {inst?.cep ? `CEP: ${inst.cep}` : 'CEP: 07044-170'} {inst?.city_uf ? ` - ${inst.city_uf}` : ' - GUARULHOS/SP'}
                </p>
                <div className="flex items-center gap-2.5 leading-none font-bold text-[8pt] pt-0.5">
                  <span className="flex items-center gap-1">
                    TEL: {inst?.phone || '(11) 2421-2935'}
                  ...
                  <span className="flex items-center gap-1">
                    EMAIL: <span className="lowercase font-bold">{inst?.email || 'email@email.com.br'}</span>
                  </span>
                </div>
              </div>
              <div className="text-right max-w-[380px] leading-tight text-black font-bold uppercase text-[7pt]">
                ...
                <p className="whitespace-pre-line lowercase font-bold text-[7.5pt]">{inst?.secretary || 'quarta a sexta-feira das 14h as 18h\ncurso básico de teologia'}</p>
              </div>
            </div>
          </div>
        </div>
