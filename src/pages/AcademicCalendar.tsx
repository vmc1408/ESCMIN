...
export function AcademicCalendar() {
  ...
  const classMatchesWeekday = (c: Class, targetDay: number | undefined | null) => {
    if (targetDay === undefined || targetDay === null) return false;
    ...
    return targetWeekdays.includes(targetDay);
  };
  ...
  const getWeekdayDataForDoc = (i: number, parsedDoc: AcademicSettings, currentEventsList?: CalendarEvent[]) => {
    ...
    // Fallback to top-level target_class_ids if day matches and no specific weekday classes exist across the document
    const hasAnyWeekdayClassesMapped = Object.keys(parsedDoc.weekday_classes || {}).some(
      k => Array.isArray(parsedDoc.weekday_classes[k]) && parsedDoc.weekday_classes[k].length > 0
    );
    if (uniqueClassIds.length === 0 && isRegisteredInWeekdays && !hasAnyWeekdayClassesMapped && parsedDoc.target_class_ids && parsedDoc.target_class_ids.length > 0) {
    ...
    // Self-healing: if title matches a term boundary label like Término/Início do Semestre/Ano Letivo, discard it and regenerate
    if (title) {
      const termTitles = ["Início do 1º", "Término do 1º", "Início do 2º", "Término do Ano Letivo", "Início do 1º Semestre", "Término do 1º Semestre", "Início do 2º Semestre"];
      if (termTitles.some(t => title.startsWith(t))) {
        title = '';
      }
    }
    ...
    try {
      ...
      if (settingsObj || localData.id) {
        ...
        setAcademicSettings({
          ...
          term2_start: mergedData.term2_start || '',
          term2_end: mergedData.term2_end || '',
          target_class_ids: mergedData.target_class_ids || (mergedData.target_class_id ? [mergedData.target_class_id] : []),
    ...
    if (!days) {
      if (selectedWeekdayDetail !== null) {
        setConfirmModalConfig({
          id: 'clear_classes_single',
          title: "Excluir Registros",
          message: (
            <span>
              Atenção: este procedimento irá excluir todos os registros de dia aula gerados para{" "}
              <span className="mx-1 px-3 py-1 bg-indigo-600 text-white font-sans font-bold text-[10px] uppercase tracking-wider inline-block select-all rounded-none shadow-sm animate-pulse">
                {['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'][selectedWeekdayDetail]}
              </span>{" "}
              no calendário. Aulas canceladas e abonadas neste dia também serão excluídas do sistema.
            </span>
          ),
          type: 'danger',
          action: async () => {
            await clearClassDays([selectedWeekdayDetail]);
          }
        });
      ...
      // No weekday has been selected
      setNotification({
        type: 'err',
        message: 'Por favor, selecione um dia da semana no painel (DOM, SEG, TER, etc.) para excluir seus registros.'
      });
      setConfirmModalConfig({
        id: 'no_weekday_selected',
        title: "Selecionar Dia Obrigatório",
        message: "Por favor, selecione primeiro um dia de aula no painel de ajustes (entre Domingo e Sábado) para poder executar a exclusão dos registros das aulas associadas.",
        type: 'info',
        action: async () => {
  ...
  const generateClassDays = async (customSettings?: any) => {
    ...
    try {
      ...
      for (let wIdx = 0; wIdx < activeWeekdays.length; wIdx++) {
        ...
        // Retrieve terms specific to this weekday or fallback to settings top-level terms!
        const dayTerms = settings.weekday_terms?.[weekdayNum] || settings.weekday_terms?.[String(weekdayNum)] || {};
        const activeT1Start = dayTerms.term1_start || settings.term1_start;
        const activeT1End = dayTerms.term1_end || settings.term1_end;
        const activeT2Start = dayTerms.term2_start || settings.term2_start;
        const activeT2End = dayTerms.term2_end || settings.term2_end;

        const weekdayRanges = [
          { start: new Date(activeT1Start + 'T00:00:00'), end: new Date(activeT1End + 'T00:00:00') },
          { start: new Date(activeT2Start + 'T00:00:00'), end: new Date(activeT2End + 'T00:00:00') }
        ];
        ...
        for (let i = 0; i < finalTargetIds.length; i++) {
          ...
          // Add Term Start/End
          const termEvents = [
            { date: activeT1Start, title: `Início do 1º Semestre${classLabel}`, type: 'start_term' },
            { date: activeT1End, title: `Término do 1º Semestre${classLabel}`, type: 'end_term' },
            { date: activeT2Start, title: `Início do 2º Semestre${classLabel}`, type: 'start_term' },
            { date: activeT2End, title: `Término do Ano Letivo${classLabel}`, type: 'end_term' }
          ];
          ...
          for (const range of weekdayRanges) {
    ...
    <>
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 no-print">
        ...
        <div className="relative md:sticky md:top-[-16px] z-20 bg-slate-50/95 backdrop-blur-md pt-2 pb-3 md:pb-5 border-b border-slate-200/80 -mx-4 px-4 space-y-2 md:space-y-4">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 md:gap-4 pb-1">
            <div className="space-y-0.5 md:space-y-1">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-lg md:text-2xl font-bold text-slate-900 tracking-tight">Cronograma Acadêmico</h1>
              </div>
              <p className="hidden md:block text-slate-500 text-[11px] md:text-xs font-medium">Gestão de ciclos letivos e atividades escolares.</p>
            </div>

            <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-start">
              <button 
                onClick={() => setShowPrintOptions(true)}
                className="flex items-center gap-1.5 px-2.5 md:px-4 py-2 bg-slate-100 text-slate-700 rounded-none text-[9px] font-bold uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all active:scale-95 border border-slate-200 h-[36px]"
              >
                <Printer size={14} />
                <span className="hidden sm:inline">Imprimir Relatórios</span>
              ...
              {(isAdmin || isDirector) && (
                <div className="flex items-center gap-2 animate-in fade-in duration-200">
                  <button 
                    ...
                    className="h-[36px] px-2.5 md:px-4 flex items-center gap-1.5 bg-slate-800 text-white hover:bg-slate-900 transition-all rounded-none text-[9px] font-bold uppercase tracking-widest active:scale-95 shadow-sm"
                    title="Novo Registro"
                  >
                    <Plus size={14} />
          ...
          {viewMode !== 'management' && (
            <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-3 animate-in fade-in duration-300">
              
              {/* Row 2.1: Navigation, View selectors and Toggle Button */}
              <div className="flex flex-wrap items-center justify-between gap-2.5 w-full lg:w-auto order-1 lg:order-2">
                ...
                <div className="flex items-center bg-white border border-slate-200 rounded-none p-1 shadow-sm w-fit animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className="px-3 min-w-[95px] text-center border-x border-slate-100 font-sans">
                    <span className="text-[10px] sm:text-[11px] font-bold text-slate-800 uppercase tracking-[0.05em] sm:tracking-[0.08em] select-none">
                      {viewMode === 'month' 
                    ...
                    {viewMode === 'month' && (
                      <span className="text-[8px] sm:text-[9px] font-bold text-slate-400 ml-1 select-none">
                        {currentDate.getFullYear()}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex bg-slate-100 p-1 rounded-none border border-slate-200/50 shadow-inner w-fit">
                  <button 
                    ...
                    className={cn(
                      "px-2 sm:px-3 py-1 sm:py-1.5 rounded-none text-[8px] sm:text-[9px] font-bold uppercase tracking-widest transition-all",
                      viewMode === 'month' ? "bg-white text-slate-800 shadow-xs border border-slate-100" : "text-slate-500 hover:text-slate-700"
                    )}
                  >
                    Mês
                  </button>
                  <button 
                    ...
                    className={cn(
                      "px-2 sm:px-3 py-1 sm:py-1.5 rounded-none text-[8px] sm:text-[9px] font-bold uppercase tracking-widest transition-all",
                      viewMode === 'year' ? "bg-white text-slate-800 shadow-xs border border-slate-100" : "text-slate-500 hover:text-slate-700"
                    )}
                  >
                    Ano
                  </button>
                  <button 
                    ...
                    className={cn(
                      "px-2 sm:px-3 py-1 sm:py-1.5 rounded-none text-[8px] sm:text-[9px] font-bold uppercase tracking-widest transition-all",
                      viewMode === 'list' ? "bg-white text-slate-800 shadow-xs border border-slate-100" : "text-slate-500 hover:text-slate-700"
                    )}
                  >
                    Lista
                  </button>
                ...
                {/* Botão de Resumo (Apenas Visível no Mobile/Tablet) */}
                <button
                  onClick={() => setShowStats(!showStats)}
                  className="lg:hidden h-[34px] px-2.5 flex items-center gap-1 bg-slate-100 hover:bg-slate-200 text-slate-600 border border-slate-200 rounded-none text-[8px] sm:text-[9px] font-bold uppercase tracking-widest transition-all active:scale-95"
                  title="Exibir Resumo de Aulas"
                >
                  <Filter size={12} className="text-slate-500" />
                  <span>{showStats ? 'Ocultar Resumo' : 'Filtrar/Resumo'}</span>
                </button>
              </div>

              {/* Status Panels (Quarta / Quinta) */}
              <div className={cn(
                "flex-wrap items-center gap-2.5 order-2 lg:order-1",
                showStats ? "flex animate-in slide-in-from-top-2 duration-200" : "hidden lg:flex"
              )}>
              ...
              {inspectingClassId ? (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                  <div className="xl:col-span-2 space-y-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <h2 className="text-xl font-bold text-slate-900 tracking-tight">Cronograma de Aulas por Turma</h2>
      ...
      <AnimatePresence>
        {isEditing && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-2 sm:p-4">
            ...
            >
              <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100 relative shrink-0">
                <button 
                  type="button"
                  onClick={() => setIsEditing(false)}
                  className="absolute top-4 sm:top-5 right-4 sm:right-6 p-1.5 hover:bg-slate-50 rounded-none transition-all text-slate-400 hover:text-slate-600"
                >
                  <X size={18} />
                </button>
                <div className="flex items-center gap-3">
                  <div className={cn(
                    "w-8 h-8 sm:w-10 sm:h-10 rounded-none flex items-center justify-center shadow-sm border",
                    getTypeStyle(formData.type)
                  )}>
                    <CalendarPlus size={16} className="sm:size-5" />
                  </div>
                  <div>
                    ...
                    <p className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 whitespace-nowrap">Gestão de Calendário</p>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-5 custom-scrollbar">
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Tipo de Registro</label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
                    {[
                      { id: 'holiday_nac', label: 'Nacional', icon: <Globe size={12} />, color: 'rose', desc: 'Feriado Federal' },
                      { id: 'holiday_mun', label: 'Municipal', icon: <MapPin size={12} />, color: 'amber', desc: 'Padroeiro/Local' },
                      { id: 'exam', label: 'Avaliação', icon: <GraduationCap size={12} />, color: 'orange', desc: 'Provas/Testes' },
                      { id: 'class_day', label: 'Aula Normal', icon: <BookOpen size={12} />, color: 'blue', desc: 'Dia Letivo' },
                      { id: 'excused_class', label: 'Aula Abonada', icon: <Divide size={12} />, color: 'slate', desc: 'Dispensa/Abono' },
                      { id: 'cancelled_class', label: 'Aula Cancelada', icon: <Ban size={12} />, color: 'red', desc: 'Aula Nula' }
                    ].map(type => (
                          <button
                            key={type.id}
                            type="button"
                            ...
                            className={cn(
                              "relative flex flex-col p-2 sm:p-2.5 rounded-none border-2 transition-all text-left group",
                          ...
                          >
                            <div className={cn(
                              "w-6 h-6 sm:w-7 sm:h-7 rounded-none flex items-center justify-center mb-1 sm:mb-1.5 transition-transform group-hover:scale-110",
                              formData.type === type.id ? `bg-${type.color === 'red' ? 'rose' : type.color}-600 text-white` : "bg-slate-100 text-slate-400"
                            )}>
                              {type.icon}
                            </div>
                            <p className={cn(
                              "text-[10px] sm:text-[11px] font-bold uppercase tracking-tight leading-tight",
                              formData.type === type.id ? `text-${type.color === 'red' ? 'rose' : type.color}-700` : "text-slate-500"
                            )}>{type.label}</p>
                            <p className="hidden xs:block text-[8px] sm:text-[8.5px] font-medium text-slate-400 leading-none mt-0.5">{type.desc}</p>
                            
                            {formData.type === type.id && (
                              <div className={cn(`absolute top-1 sm:top-1.5 right-1 sm:right-1.5 w-3 h-3 sm:w-3.5 sm:h-3.5 flex items-center justify-center rounded-full bg-${type.color === 'red' ? 'rose' : type.color}-600 shadow-sm shadow-${type.color === 'red' ? 'rose' : type.color}-200`)}>
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1 block">Período</label>
                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                      <div className="relative">
                        <input 
                          ...
                          className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-slate-50 border border-slate-200 rounded-none text-xs font-bold text-slate-700 focus:ring-4 focus:ring-slate-100 focus:bg-white focus:border-slate-400 transition-all outline-none"
                        />
                        <span className="absolute -top-1.5 left-2 px-1 bg-white text-[6.5px] font-bold text-slate-400 uppercase border border-slate-100 rounded font-sans">Início</span>
                      </div>
                      <div className="relative">
                        <input 
                          readOnly={!(isAdmin || isDirector)}
                          ...
                          className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-slate-50 border border-slate-200 rounded-none text-xs font-bold text-slate-700 focus:ring-4 focus:ring-slate-100 focus:bg-white focus:border-slate-400 transition-all outline-none"
                        />
                        <span className="absolute -top-1.5 left-2 px-1 bg-white text-[6.5px] font-bold text-slate-400 uppercase border border-slate-100 rounded font-sans">Término</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div className="space-y-1.5">
                        <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1 block mb-1 font-sans">Escopo do Registro</label>
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            ...
                            className={cn(
                              "py-2 sm:py-2.5 px-2 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider border rounded-none transition-all flex items-center justify-center gap-1",
                              editScope === 'all'
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1 block font-sans">
                          {editScope === 'all' ? 'Dia de Aula Afetado' : 'Vincular Turma'}
                        </label>
                        {editScope === 'all' ? (
                          <div className="w-full px-3 py-2 sm:py-2.5 bg-slate-50 border border-slate-100 rounded-none text-xs font-bold text-slate-400 cursor-not-allowed uppercase">
                ...
                <div className="flex gap-2 pt-3 border-t border-slate-150 shrink-0">
                  {selectedEvent && (isAdmin || isDirector) && (
                    <button 
                      ...
                      className="p-2 sm:p-3 rounded-none transition-all border bg-white text-red-500 border-red-100 hover:bg-red-50 hover:border-red-200 shadow-sm"
                      title="Excluir Lançamento"
                    >
                      <Trash2 size={16} />
                    </button>
                  ...
                  <button 
                    type="button"
                    onClick={() => setIsEditing(false)}
                    className="flex-1 py-2 sm:py-3 px-3 bg-white border border-slate-200 text-slate-500 rounded-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm"
                  >
                    Fechar
                  </button>
                  ...
                  {(isAdmin || isDirector) && (
                    <button 
                      ...
                      className={cn(
                        "flex-[2] py-2 sm:py-3 px-3 rounded-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-1.5",
                        (isSyncing || !isFormChanged)
                          ? "bg-slate-300 text-slate-500 cursor-not-allowed shadow-none"
                          : "bg-slate-800 text-white hover:bg-slate-900 shadow-xl"
                      )}
                    >
                      {isSyncing ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />}
            ...
            >
              ...
              <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 relative bg-white shrink-0">
                ...
                <div className="flex items-center gap-2.5 sm:gap-3">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 bg-slate-50 text-slate-600 rounded flex items-center justify-center border border-slate-100">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                  ...
                  <div className="lg:col-span-5 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-4 sm:grid-cols-7 gap-1 sm:gap-1.5">
                        {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab'].map((day, i) => {
                          ...
                          // Dynamically evaluate if this day has actual custom rule / personalized parameters saved
                          const activeTerms = settingsForm.weekday_terms?.[i] || settingsForm.weekday_terms?.[String(i)];
                          const hasCustomDates = !!(activeTerms && (activeTerms.term1_start || activeTerms.term1_end || activeTerms.term2_start || activeTerms.term2_end));
                          
                          const activeDayClasses = settingsForm.weekday_classes?.[i] || settingsForm.weekday_classes?.[String(i)];
                          const hasCustomClasses = activeDayClasses !== undefined && activeDayClasses !== null && activeDayClasses.length > 0;
                          
                          const hasCustomRule = hasCustomDates || hasCustomClasses;
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                const nextActiveDay = i;
                                
                                // Retrieve classes already mapped for this weekday or normal associations for display target
                                const savedWeekdayClasses = settingsForm.weekday_classes?.[nextActiveDay] 
                                  || settingsForm.weekday_classes?.[String(nextActiveDay)]
                                  || academicSettings.weekday_classes?.[nextActiveDay]
                                  || academicSettings.weekday_classes?.[String(nextActiveDay)];

                                const associatedClassIds = classes
                                  .filter(c => classMatchesWeekday(c, nextActiveDay))
                                  .map(c => c.id);

                                const nextClassIds = savedWeekdayClasses !== undefined && savedWeekdayClasses !== null
                                  ? savedWeekdayClasses
                                  : associatedClassIds;
                              ...
                              className={cn(
                                "py-2 px-1.5 rounded-none border flex flex-col items-center gap-1 transition-all relative cursor-pointer",
                                isSelected 
                            ...
                            >
                              ...
                              <div className={cn(
                                ...
                                isSelected ? "bg-blue-400" : isAlreadyRegistered ? "bg-indigo-600" : "bg-slate-200"
                              )} />
                              {hasCustomRule && (
                                <span className={cn(
                                  "absolute top-0.5 right-0.5 text-[6px] font-bold uppercase tracking-tighter",
                    ...
                    {selectedWeekdayDetail !== null ? (() => {
                      const activeDay = selectedWeekdayDetail;
                      const activeTerms = settingsForm.weekday_terms?.[activeDay] || settingsForm.weekday_terms?.[String(activeDay)] || {};
                      
                      const isDayActiveInForm = (settingsForm.class_weekdays || []).includes(activeDay);
                      const hasCustomDates = !!(activeTerms.term1_start || activeTerms.term1_end || activeTerms.term2_start || activeTerms.term2_end);

                      if (!isDayActiveInForm) {
                        return (
                          <div className="border border-dashed border-slate-200 bg-slate-50/50 p-5 text-center space-y-2 py-8 animate-in fade-in duration-300">
                            <CalendarDays className="text-slate-300 mx-auto" size={24} />
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-normal">
                              Período Letivo Indisponível
                            </p>
                            <p className="text-[10px] text-slate-400 leading-normal max-w-[200px] mx-auto mt-1 font-sans">
                              Este dia está desativado. Ative este dia da semana na agenda ao lado para configurar seus períodos letivos próprios.
                            </p>
                          </div>
                        );
                      }

                      // Render empty custom inputs directly for activated weekdays
                      return (
                        <div className="border border-slate-200 bg-white p-3.5 space-y-3 animate-in fade-in duration-300">
                          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                            <div className="space-y-0.5">
                              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block font-sans">
                                Períodos Letivos - {['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'][activeDay]}
                              </h4>
                              <p className="text-[10px] text-emerald-600 font-bold leading-none uppercase tracking-wide">⚙️ CONFIGURAR CRONOGRAMA</p>
                            </div>
                            {hasCustomDates && (
                              <button
                                type="button"
                                onClick={() => {
                                  const updatedTerms = { ...(settingsForm.weekday_terms || {}) };
                                  delete updatedTerms[activeDay];
                                  delete updatedTerms[String(activeDay)];
                                  setSettingsForm({
                                    ...settingsForm,
                                    weekday_terms: updatedTerms
                                  });
                                }}
                                className="py-1 px-2 border border-slate-100 hover:bg-slate-50 hover:text-red-600 text-slate-400 text-[8.5px] font-bold uppercase tracking-wider transition-all cursor-pointer font-sans"
                                title="Limpar datas deste dia"
                              >
                                Limpar Datas
                              </button>
                            )}
                          ...
                          <div className="space-y-3">
                            {[
                              ...
                              const valStartRaw = t.term === 1 ? (activeTerms.term1_start || '') : (activeTerms.term2_start || '');
                              const valEndRaw = t.term === 1 ? (activeTerms.term1_end || '') : (activeTerms.term2_end || '');

                              const valStart = parseDateToDB(valStartRaw) || '';
                              const valEnd = parseDateToDB(valEndRaw) || '';
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    <div className="space-y-0.5">
                                      ...
                                      <input 
                                        ...
                                        onChange={e => {
                                          const updatedTerms = { ...(settingsForm.weekday_terms || {}) };
                                          const currentTerms = updatedTerms[activeDay] || updatedTerms[String(activeDay)] || {};
                                          updatedTerms[activeDay] = {
                                            ...currentTerms,
                                        ...
                                        className="un-styled-input w-full border border-slate-200 rounded-none py-1.5 px-2 text-xs font-bold bg-white text-slate-700 focus:border-indigo-300 outline-none transition-all"
                                      />
                                    </div>
                                    <div className="space-y-0.5">
                          ...
                          <p className="text-[8.5px] text-slate-400 leading-normal font-sans italic mt-1">
                            * Se deixados em branco, o sistema usará o período padrão geral do calendário anual ao gerar as aulas.
                          </p>
                        </div>
                      );
                    })() : (
                      <div className="border border-dashed border-slate-200 bg-white p-6 text-center">
                        <CalendarDays className="text-slate-300 mx-auto mb-2" size={24} />
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-normal text-slate-500 font-sans">
                          Período Letivo
                        </p>
                        <p className="text-[9px] text-slate-400 leading-normal max-w-[150px] mx-auto mt-1 font-sans">
                          Selecione um dia de aula acima para habilitar as datas.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="lg:col-span-7 flex flex-col justify-between space-y-3 self-stretch">
                    <div className="space-y-3 flex-1">
                      {selectedWeekdayDetail !== null ? (
                        <div className="space-y-3 animate-in fade-in duration-300">
                          {/* Status de Registro Existente com Toggle Direto */}
                          {(() => {
                            const activeDay = selectedWeekdayDetail;
                            const isDayActiveInForm = (settingsForm.class_weekdays || []).includes(activeDay);
                            return (
                              <div className={cn(
                                "p-3 rounded-none flex items-center justify-between gap-3 border transition-all duration-300",
                                isDayActiveInForm 
                                  ? "bg-indigo-50/40 border-indigo-150/80 shadow-inner" 
                                  : "bg-slate-50 border-slate-200"
                              )}>
                                <div className="flex items-center gap-2">
                                  <span className={cn(
                                    "w-2 h-2 rounded-full",
                                    isDayActiveInForm ? "bg-indigo-600 animate-pulse" : "bg-slate-300"
                                  )} />
                                  <div>
                                    <span className="font-bold text-slate-800 text-[10px] uppercase tracking-wide block">
                                      Agenda das Aulas de {['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'][activeDay]}
                                    </span>
                                    <span className="text-[10px] text-slate-500 font-medium leading-tight block mt-0.5">
                                      {isDayActiveInForm 
                                        ? (() => {
                                            const hasSavedRecords = events.some(e => {
                                              if (e.description?.includes('Cronograma automático') && e.start_date) {
                                                const dt = new Date(e.start_date + 'T00:00:00');
                                                return dt.getDay() === activeDay;
                                              }
                                              return false;
                                            });
                                            ...
                                            if (hasSavedRecords) {
                                              return (
                                                <span className="text-red-600 font-semibold block bg-red-50/60 p-1.5 border border-red-100/50 mt-1">
                                                  ⚠️ Existem registros salvos para este dia de aula. Exclua os registros deste dia se quiser desativá-lo.
                                                </span>
                                              );
                                            }
                                            return "Este dia está ativado e as aulas automáticas serão geradas.";
                                        })()
                                        : "Este dia está desativado. Nenhuma aula será gerada para este dia."}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    const isAlreadyActive = (settingsForm.class_weekdays || []).includes(activeDay);
                                    
                                    if (isAlreadyActive) {
                                      // Prevent deactivation if there are saved records
                                      const hasSavedRecords = events.some(e => {
                                        if (e.description?.includes('Cronograma automático') && e.start_date) {
                                          const dt = new Date(e.start_date + 'T00:00:00');
                                          return dt.getDay() === activeDay;
                                      ...
                                      if (hasSavedRecords) {
                                        setNotification({
                                          type: 'err',
                                          message: 'Não é permitido desativar este dia porque existem registros de aula salvos. Vá em "Excluir Registros" e remova as aulas deste dia primeiro.'
                                        });
                                        return;
                                      }
                                    }

                                    const updatedWeekdays = isAlreadyActive
                                      ? (settingsForm.class_weekdays || []).filter(x => x !== activeDay)
                                      : [...(settingsForm.class_weekdays || []), activeDay];
                                    
                                    const updatedWeekdayTerms = { ...(settingsForm.weekday_terms || {}) };
                                    let nextT1Start = '';
                                    let nextT1End = '';
                                    let nextT2Start = '';
                                    let nextT2End = '';
                                    ...
                                    if (isAlreadyActive) {
                                      // Deactivating: clear terms for this day
                                    ...
                                    setSettingsForm({
                                      ...
                                      weekday_terms: updatedWeekdayTerms
                                    });
                                  }}
                                  className={cn(
                                    "font-bold py-1.5 px-3 rounded-none text-[9px] uppercase tracking-wider transition-all border cursor-pointer shrink-0",
                                    isDayActiveInForm
                                      ? "bg-red-50 hover:bg-red-100 text-red-600 border-red-200"
                                      : "bg-indigo-600 hover:bg-indigo-700 text-white border-indigo-600 shadow-md shadow-indigo-100/30"
                                  )}
                                >
                                  {isDayActiveInForm ? 'Desativar Dia' : 'Ativar Dia'}
                                </button>
                              </div>
                            );
                          })()}

                           {/* CONFIGURAÇÃO DO EVENTO DESTE DIA */}
                           {(() => {
                             const activeDay = selectedWeekdayDetail;
                             const isDayActiveInForm = (settingsForm.class_weekdays || []).includes(activeDay);
                             return (
                               <div className={cn(
                                 "bg-white p-4 border border-slate-200/85 space-y-3 transition-all duration-300 relative",
                                 !isDayActiveInForm && "opacity-45 pointer-events-none select-none"
                               )}>
                                 {!isDayActiveInForm && (
                                   <div className="absolute inset-0 bg-slate-50/10 z-10 flex items-center justify-center p-4">
                                     <span className="bg-slate-950 text-white text-[8px] font-bold uppercase tracking-wider px-2 py-1 shadow-sm">
                                       ⚠️ Ative este dia acima para configurar
                                     </span>
                                   </div>
                                 )}
                                 {/* Campo Título */}
                                 <div className="space-y-1">
                                   <div className="flex justify-between items-center">
                                     <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                       Título da Aula para {['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'][activeDay]}
                                     </label>
                                     <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                                       {(((settingsForm.weekday_titles || {})[activeDay] !== undefined 
                                         ? (settingsForm.weekday_titles || {})[activeDay] 
                                         : (settingsForm.weekday_titles || {})[String(activeDay)]) || '').length}/45
                                     </span>
                                   </div>
                                   <input 
                                     type="text"
                                     maxLength={45}
                                     placeholder={`Ex: Aula, Classe(s) Turma(s) de ${['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'][activeDay]}...`}
                                     value={(settingsForm.weekday_titles || {})[activeDay] !== undefined 
                                       ? (settingsForm.weekday_titles || {})[activeDay] 
                                       : ((settingsForm.weekday_titles || {})[String(activeDay)] || '')}
                                   ...
                                   <div className="flex items-center justify-between">
                                     ...
                                     <div className="flex gap-2">
                                         ...
                                         onClick={() => {
                                           ...
                                           setSettingsForm({ 
                                             ...
                                             weekday_classes: {
                                               ...
                                               [activeDay]: filteredClassIds
                                             }
                                           });
                                         }}
                                         className="text-[9px] font-bold text-slate-800 uppercase tracking-wider hover:underline font-sans cursor-pointer"
                                       >
                                         Toda Turma
                                       </button>
                                       ...
                                       <button 
                                         ...
                                         className="text-[9px] font-bold text-slate-400 uppercase tracking-wider hover:underline font-sans cursor-pointer"
                                       >
                                         Limpar
                                       </button>
                                     </div>
                                           ...
                                           <button
                                             ...
                                             onClick={() => {
                                               const nextClassIds = isSelected
                                                 ? settingsForm.target_class_ids.filter(id => id !== c.id)
                                                 : [...settingsForm.target_class_ids, c.id];
                                               setSettingsForm({
                                                 ...settingsForm,
                                                 target_class_ids: nextClassIds,
                                                 weekday_classes: {
                                                   ...(settingsForm.weekday_classes || {}),
                                                   [activeDay]: nextClassIds
                                                 }
                                               });
                                             }}
                                             className={cn(
                                               "inline-flex items-center gap-1 px-2 py-1 text-[9px] font-bold border transition-all rounded-full cursor-pointer",
                                               isSelected 
                        ...
                        <div className="h-auto min-h-[160px] py-6 flex flex-col items-center justify-center border border-dashed border-slate-200 bg-white text-center space-y-1.5 shadow-sm rounded-none">
                          <CalendarDays className="text-slate-200 animate-pulse" size={32} />
                          <div>
                            <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest leading-normal">
                              Nenhum Dia Selecionado
                            </p>
                            <p className="text-[9px] text-slate-400 font-medium leading-normal max-w-[240px] mx-auto mt-0.5">
                              Selecione primeiro um dia de aula no painel ao lado para habilitar as turmas associadas e os períodos letivos.
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    {/* Feriados do Calendário Frame/Card */}
                    {(() => {
                      const curYear = currentDate.getFullYear();
                      const holidaysForYear = events.filter(e => {
                        const d = new Date(e.start_date + 'T00:00:00');
                        const isHoliday = e.type === 'holiday' || e.type === 'holiday_nac' || e.type === 'holiday_est' || e.type === 'holiday_mun';
                        return isHoliday && d.getFullYear() === curYear;
                      });
                      const countHolidays = holidaysForYear.length;

                      return (
                        <div className="border border-slate-200 bg-white p-3 space-y-2.5 shadow-xs rounded-none">
                          <div className="flex items-center gap-2 border-b border-slate-100 pb-1.5">
                            <div className="w-5 h-5 bg-indigo-50 text-indigo-600 rounded-none flex items-center justify-center border border-indigo-100">
                              <RefreshCw size={10} />
                            </div>
                            <div>
                              ...
                              <p className="text-[8px] text-slate-400 font-bold uppercase tracking-wider leading-none">
                                Base de Feriados de {curYear}
                              </p>
                            </div>
                          </div>

                          <div className="bg-slate-50 p-2 border border-slate-100 rounded-none">
                            <p className="text-[10px] text-slate-500 leading-normal font-sans">
                              {countHolidays > 0 ? (
                                <>
                                  Atualmente existem <strong className="text-indigo-600">{countHolidays}</strong> feriados nacionais e facultativos sincronizados para o ano de <strong>{curYear}</strong>.
                                </>
                              ) : (
                                <>
                                  Você ainda não sincronizou os feriados nacionais para o ano de <strong>{curYear}</strong>.
                                </>
                              )}
                            </p>
                          </div>
                          ...
                          <button
                            ...
                            disabled={isSyncing}
                            className="w-full py-1.5 px-3 bg-indigo-50 hover:bg-slate-100 border border-indigo-150 text-indigo-750 hover:text-indigo-850 transition-all text-[9px] font-bold uppercase tracking-wider cursor-pointer font-sans flex items-center justify-center gap-1.5 disabled:opacity-50"
                          >
                            <RefreshCw size={10} className={cn(isSyncing && "animate-spin")} />
                            Sincronizar Feriados Reais
                          </button>
                        </div>
                      );
                    })()}
              ...
              {/* Botões do Rodapé */}
              <div className="px-4 sm:px-6 py-3 sm:py-4 bg-slate-50 border-t border-slate-150 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 shrink-0">
                <div>
                  <button 
                    onClick={() => {
                      setShowSettings(false);
                    ...
                    className="w-full sm:w-auto px-4 py-2 sm:px-5 sm:py-2.5 bg-white border border-slate-350 hover:bg-slate-100 text-slate-700 hover:text-slate-900 rounded-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                  >
                ...
                <div className="flex flex-col xs:flex-row items-stretch xs:items-center gap-2 justify-end">
                  {(isAdmin || isDirector) && (
                    <button 
                      ...
                      disabled={isSyncing}
                      className="px-3 py-2 sm:px-4 sm:py-2.5 bg-white border-2 border-red-500 hover:bg-red-50 text-red-600 rounded-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 shadow-sm cursor-pointer"
                    >
                      <Trash2 size={12} />
                      Excluir Registros
                    </button>
                  ...
                  <button 
                    onClick={() => {
                      const activeDay = selectedWeekdayDetail ?? 3;
                      const isDayActive = (settingsForm.class_weekdays || []).includes(activeDay);

                      // Root fallbacks
                      const rootT1Start = parseDateToDB(settingsForm.term1_start) || parseDateToDB(academicSettings.term1_start) || '';
                      const rootT1End = parseDateToDB(settingsForm.term1_end) || parseDateToDB(academicSettings.term1_end) || '';
                      const rootT2Start = parseDateToDB(settingsForm.term2_start) || parseDateToDB(academicSettings.term2_start) || '';
                      const rootT2End = parseDateToDB(settingsForm.term2_end) || parseDateToDB(academicSettings.term2_end) || '';

                      if ((settingsForm.class_weekdays || []).length === 0) {
                        setNotification({ type: 'err', message: 'Selecione pelo menos um dia da semana para ter aulas no calendário.' });
                        return;
                      }

                      if (isDayActive && (!settingsForm.target_class_ids || settingsForm.target_class_ids.length === 0)) {
                        setNotification({ type: 'err', message: 'Selecione pelo menos uma turma para as aulas deste dia ativado.' });
                        return;
                      }

                      setConfirmModalConfig({
                        id: 'save_academic_settings',
                        type: 'danger',
                        title: 'Confirmar Geração das Aulas',
                        message: 'Tem certeza de que deseja atualizar as configurações letivas e recalcular as aulas automáticas no calendário? Esta ação recalculará as datas com base nos períodos letivos e turmas associadas a cada dia da semana. Todas as aulas automáticas anteriores serão excluídas e regeneradas.',
                        action: async () => {
                          setShowConfirmModal(false);
                          setIsSyncing(true);
                          try {
                            ...
                            // Build final merged terms safely preserving every day's choices
                            const finalFormTerms = {
                            ...
                            if (selectedWeekdayDetail !== null) {
                              ...
                              const hasCustom = !!(activeDayTerms.term1_start || activeDayTerms.term1_end || activeDayTerms.term2_start || activeDayTerms.term2_end);
                              
                              if (hasCustom) {
                                finalFormTerms[selectedWeekdayDetail] = {
                                  term1_start: parseDateToDB(activeDayTerms.term1_start),
                                  term1_end: parseDateToDB(activeDayTerms.term1_end),
                                  term2_start: parseDateToDB(activeDayTerms.term2_start),
                                  term2_end: parseDateToDB(activeDayTerms.term2_end),
                                };
                              } else {
                                delete finalFormTerms[selectedWeekdayDetail];
                                delete finalFormTerms[String(selectedWeekdayDetail)];
                              }
                            }

                            const nextWeekdayClasses: Record<number, string[]> = {};
                            const nextWeekdayTerms: Record<number, any> = {};
                            ...
                            nextWeekdays.forEach(d => {
                              ...
                              const classesForDay = finalFormClasses[d] 
                                ...
                                || [];
                              nextWeekdayClasses[d] = classesForDay;

                              // Retrieve and store merged terms for this day ONLY if actual custom dates are present
                              const termsForDay = finalFormTerms[d] 
                                || finalFormTerms[String(d)];
                              
                              if (termsForDay && (termsForDay.term1_start || termsForDay.term1_end || termsForDay.term2_start || termsForDay.term2_end)) {
                                nextWeekdayTerms[d] = {
                                  term1_start: parseDateToDB(termsForDay.term1_start),
                                  term1_end: parseDateToDB(termsForDay.term1_end),
                                  term2_start: parseDateToDB(termsForDay.term2_start),
                                  term2_end: parseDateToDB(termsForDay.term2_end)
                                };
                              }

                              // Retrieve and store merged titles for this day
                              const titleForDay = settingsForm.weekday_titles?.[d]
                                || settingsForm.weekday_titles?.[String(d)]
                                || academicSettings.weekday_titles?.[d]
                                || academicSettings.weekday_titles?.[String(d)]
                                || `Aula, Classe(s) Turma(s) de ${['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado'][d]}`;
                            ...
                            Object.values(nextWeekdayClasses).forEach((classesList) => {
                              if (Array.isArray(classesList)) {
                                classesList.forEach(cid => {
                                  if (cid) allClassIdsSet.add(cid);
                                });
                              }
                            });
                            
                            for (const cid of Array.from(allClassIdsSet)) {
                              await saveData('academic_settings', cid, updatedSettings);
                              try {
                                localStorage.setItem(`academic_settings_${cid}`, JSON.stringify(updatedSettings));
                              } catch (e) {
                                console.warn("Storage write error for class:", e);
                              }
                            }
                    ...
                    disabled={isSyncing}
                    className="px-3 py-2 sm:px-5 sm:py-2.5 bg-slate-900 text-white rounded-none text-[9px] sm:text-[10px] font-bold uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 shadow-md shadow-slate-100 cursor-pointer"
                  >
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[300] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 text-center space-y-6">
                ...
                <div className={cn(
                  confirmModalConfig.id === 'no_weekday_selected' ? "flex pt-2" : "grid grid-cols-2 gap-3 pt-2"
                )}>
                  {confirmModalConfig.id === 'no_weekday_selected' ? (
                    <button 
                      onClick={() => setShowConfirmModal(false)}
                      className="w-full py-4 bg-slate-900 text-white rounded-none text-[10px] font-bold uppercase tracking-widest hover:bg-slate-800 transition-all active:scale-95 text-center shadow-md shadow-slate-100"
                    >
                      Entendido, Voltar
                    </button>
                  ) : (
