...
export function Courses() {
  ...
  const [formData, setFormData] = useState<Partial<Course>>({
    ...
    meeting_days: [],
    status: 'Ativo',
    unit_id: globalUnitId && globalUnitId !== 'all' ? globalUnitId : 'all'
  });
  ...
  // Scoped classes and students strictly matching user's selected unit
  ...
  const scopedStudents = useMemo(() => {
    if (!globalUnitId || globalUnitId === 'all') return students;
    return students.filter(s => isItemInUnit(getItemUnitId(s), globalUnitId, activeUnits));
  }, [students, globalUnitId, activeUnits]);

  // Check if a course belongs to or is offered in the active unit
  const isCourseInActiveUnit = useCallback((c: Course) => {
    const courseUnitId = getItemUnitId(c);
    // Cursos globais institucionais pertencem a todas as unidades
    if (courseUnitId === 'all' || courseUnitId === 'todas' || courseUnitId === 'global') {
      return true;
    }
    if (!globalUnitId || globalUnitId === 'all') {
      return true;
    }
    // Se possui unit_id específico e bate com a unidade ativa
    if (courseUnitId && isItemInUnit(courseUnitId, globalUnitId, activeUnits)) {
      return true;
    }
    // Se o curso possui turmas cadastradas nesta unidade ativa
    const hasClassesInActiveUnit = scopedClasses.some(cls => {
      const detected = detectCourseFromClass(cls, courses);
      return (
        (cls.course && cls.course.trim().toLowerCase() === c.name.trim().toLowerCase()) ||
        detected.toLowerCase() === c.name.toLowerCase()
      );
    });
    if (hasClassesInActiveUnit) {
      return true;
    }
    // Cursos legados sem unit_id pertencem à matriz por padrão
    if (!courseUnitId) {
      return isItemInUnit('matriz', globalUnitId, activeUnits);
    }
    return false;
  }, [globalUnitId, activeUnits, scopedClasses, courses]);

  // Check if the current user has permission to edit or delete this specific course
  const canManipulateCourse = useCallback((c: Course) => {
    if (!canEdit) return false;
    // Administrador ou usuário com permissão de alternar polos pode gerenciar qualquer curso
    if (canSwitchUnit) return true;

    // Usuário restrito ao polo logado:
    const courseUnitId = getItemUnitId(c);
    // Cursos globais ou da Matriz não podem ser alterados por usuários restritos a polos filiais
    if (!courseUnitId || courseUnitId === 'all' || courseUnitId === 'matriz') {
      return globalUnitId === 'matriz';
    }
    // Só pode gerenciar se o curso for especificamente do seu polo
    return isItemInUnit(courseUnitId, globalUnitId, activeUnits);
  }, [canEdit, canSwitchUnit, globalUnitId, activeUnits]);

  // Statistics calculation - Scoped to the active unit
  const { courseStats, courseStudentsMap, totalActiveInCourses, unassignedActiveStudents } = useMemo(() => {
    ...
    const classesMap = new Map<string, Class>(scopedClasses.map(c => [c.id, c]));
  ...
  }, [courses, scopedClasses, scopedStudents, enrollments]);

  // Filter and logically group active vs inactive, strictly adhering to unit scope
  const { activeCourses, inactiveCourses, coursesInActiveUnitCount } = useMemo(() => {
    ...
    const filtered = courses.filter(c => {
      ...
      const matchesUnit = (() => {
        if (globalUnitId && globalUnitId !== 'all') {
          return isCourseInActiveUnit(c);
        }
        if (selectedUnitFilter !== 'Todos') {
          const cUnit = getItemUnitId(c);
          if (cUnit === 'all') return true;
          return isItemInUnit(cUnit, selectedUnitFilter, activeUnits);
        }
        return true;
      })();
  ...
  const handleOpenCreate = () => {
    ...
    setFormData({
      ...
      meeting_days: [],
      status: 'Ativo',
      unit_id: globalUnitId && globalUnitId !== 'all' ? globalUnitId : (activeUnits[0]?.id || 'matriz')
    });
    setShowModal(true);
  };
  ...
  const handleOpenEdit = (course: Course) => {
    if (!canManipulateCourse(course)) {
      showNotification('error', 'Permissão negada: Este curso é institucional ou pertence a outra unidade e não pode ser editado neste polo.');
      return;
    }
    ...
    setFormData({
      ...
      unit_id: course.unit_id || (globalUnitId && globalUnitId !== 'all' ? globalUnitId : 'all')
    });
    setShowModal(true);
  };
  ...
  const handleToggleStatus = async (course: Course) => {
    if (!canManipulateCourse(course)) {
      showNotification('error', 'Permissão negada: Este curso pertence à Matriz ou a outro polo e não pode ter seu status alterado neste polo.');
      return;
    }
    ...
    try {
      const updatedPayload: Course = {
        ...
        unit_id: course.unit_id || (globalUnitId && globalUnitId !== 'all' ? globalUnitId : 'matriz'),
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingCourse && !canManipulateCourse(editingCourse)) {
      showNotification('error', 'Permissão negada: Este curso pertence à Matriz ou a outra unidade e não pode ser modificado por este polo.');
      return;
    }
    ...
    try {
      ...
      const targetUnitId = formData.unit_id || (globalUnitId && globalUnitId !== 'all' ? globalUnitId : 'all');
  ...
  const renderCourseCard = (course: Course) => {
    ...
    const isInactive = course.status === 'Inativo';
    const canManipulate = canManipulateCourse(course);
      ...
      >
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-start justify-between gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className={cn(
                    ...
                    !course.unit_id || course.unit_id === 'all'
                      ? "bg-slate-100 text-slate-600 border-slate-200"
                      : "bg-blue-50 text-blue-700 border-blue-200"
                  )}>
                    {!course.unit_id || course.unit_id === 'all'
                      ? '🌐 Geral'
                      : `🏢 ${getUnitName(activeUnits, course.unit_id)}`}
                  </span>
                </div>
              </div>
            </div>
            ...
            <button
              onClick={() => canManipulate && handleToggleStatus(course)}
              disabled={!canManipulate}
              title={canManipulate ? "Clique para alternar o status" : "Status gerenciado pela Matriz"}
              className={cn(
                "text-[9px] font-black uppercase px-2 py-0.5 border tracking-wider transition-all shrink-0",
                course.status === 'Ativo' 
                  ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100" 
                  : "bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100",
                !canManipulate && "opacity-75 cursor-not-allowed"
              )}
            >
              {course.status}
            </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/50 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                ...
                <th className="py-3 px-4">Unidade / Polo</th>
              ...
              {list.map(course => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4">
                      <span className={cn(
                        "inline-flex items-center gap-1 px-2 py-0.5 text-[10.5px] font-bold border rounded-xs",
                        !course.unit_id || course.unit_id === 'all'
                          ? "bg-slate-100 text-slate-600 border-slate-200"
                          : "bg-blue-50 text-blue-700 border-blue-200"
                      )}>
    ...
    <div id="courses-page-container" className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 p-4 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-900 mt-2">{coursesInActiveUnitCount}</p>
          <div className="flex items-center gap-3 text-[11px] text-slate-500 mt-1">
            <span className="text-emerald-700 font-bold">{activeCourses.length} ativos</span>
            <span>•</span>
            <span className="text-slate-500">{inactiveCourses.length} inativos</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              {globalUnitId && globalUnitId !== 'all' ? 'Turmas no Polo' : 'Total de Turmas'}
            </span>
            <Layers className="w-4 h-4 text-slate-500" />
          </div>
          <p className="text-2xl font-black text-slate-900 mt-2">{scopedClasses.length}</p>
          <p className="text-[11px] text-slate-500 mt-1">
            {globalUnitId && globalUnitId !== 'all' ? 'Vinculadas a esta unidade' : 'Distribuídas nos cursos'}
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              {globalUnitId && globalUnitId !== 'all' ? 'Alunos no Polo' : 'Alunos Ativos'}
            </span>
            <Users className="w-4 h-4 text-slate-500" />
          </div>
          <p className="text-2xl font-black text-slate-900 mt-2">
            {scopedStudents.filter(s => s.status === 'Ativo').length}
          </p>
      ...
      <div className="bg-white border border-slate-200 p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
          {/* Unit Filter (shown when viewing all units or when user can switch) */}
          {(!globalUnitId || globalUnitId === 'all') && hasMultipleUnits && (
            <div className="flex items-center gap-1.5">
              <Building2 className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <select
                id="course-unit-filter-select"
                value={selectedUnitFilter}
                onChange={(e) => setSelectedUnitFilter(e.target.value)}
                className="px-2.5 py-1.5 bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-700 focus:outline-none focus:border-slate-800"
              >
        ...
        <div id="course-form-modal" className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 shadow-xl max-w-lg w-full">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              {/* Polo / Unidade vinculada */}
              <div className="space-y-1 pt-2 border-t border-slate-100">
                <label className="text-xs font-semibold text-slate-700 flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <Building2 className="w-3.5 h-3.5 text-slate-500" />
                    Unidade / Polo Responsável
                  </span>
                  {isRestricted && (
                    <span className="text-[10px] text-slate-400 font-normal">Fixo no seu polo</span>
                  )}
                </label>
                {canSwitchUnit ? (
                  <select
                    ...
                    className="w-full px-3 py-2 border border-slate-300 text-xs font-medium text-slate-900 bg-white focus:outline-none focus:border-slate-800 focus:ring-1 focus:ring-slate-800"
                  >
                    <option value="all">🌐 Todas as Unidades (Geral / Institucional)</option>
                    {activeUnits.map(unit => (
                      <option key={unit.id} value={unit.id}>
                        🏢 {unit.name} ({unit.code})
                      </option>
                  ...
                  <div className="w-full px-3 py-2 border border-slate-200 bg-slate-50 text-xs font-semibold text-slate-700 flex items-center justify-between">
                    <span>🏢 {getUnitName(activeUnits, globalUnitId || 'matriz')}</span>
                    <span className="text-[10px] text-slate-400">Polo atual</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 bg-blue-100 text-blue-800 text-[10px] font-bold border border-blue-200">
                      {globalUnitId && globalUnitId !== 'all'
                        ? `Polo: ${getUnitName(activeUnits, globalUnitId)}`
                        : 'Todas as Unidades'}
                    </span>
                  </div>
                  <p className="text-[11px] font-medium text-slate-500 mt-0.5">
                    Lista nominal de alunos matriculados neste curso formativo no polo ativo
                  </p>
                </div>
              </div>
