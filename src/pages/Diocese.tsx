...
export function Diocese() {
  ...
  const handleDelete = async (item: any) => {
    if (!item?.id) {
      setNotification({ type: 'error', message: 'Erro: Identificador ausente para exclusão.' });
      return;
    }
    
    if (!window.confirm(`Excluir ${item.name || 'este registro'} permanentemente?`)) return;
    
    try {
      setLoading(true);
      const collection = activeTab === 'foranias' ? 'foraries' : activeTab === 'parishes' ? 'parishes' : 'clergy_leity';
      
      console.log(`[handleDelete] Iniciando exclusão de ${item.id} (code: ${item.code}) na coleção ${collection}`);
      // Pass code as secondary ID to ensure cleanup in dual-base environments where IDs might differ
      const success = await deleteData(collection, item.id, item.code);
      
      if (success) {
        setNotification({ type: 'success', message: 'Registro excluído!' });
        // Pequeno atraso para garantir que as bases sincronizaram
        setTimeout(() => {
          fetchData();
        }, 500);
      } else {
        setNotification({ type: 'error', message: 'Erro: O registro não pôde ser excluído de nenhuma base.' });
      }
    } catch (error) {
  ...
  const filteredItems = (activeTab === 'foranias' ? foraries : activeTab === 'parishes' ? parishes : clergy).filter(item => 
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className={cn("lg:col-span-2 space-y-4", isEditing && "hidden lg:block")}>
          <div className="flex items-center gap-4 bg-white p-4 rounded-3xl shadow-sm border border-slate-100 print:hidden">
            ...
            />
            <div className="px-3 py-1 bg-slate-50 text-slate-400 rounded-lg text-[10px] font-black uppercase tracking-tighter">
                ...
                <div key={item.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all group relative overflow-hidden print:border-slate-300 print:mb-4 print:shadow-none">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-5">
                      <div className="w-14 h-14 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors shrink-0">
                        {activeTab === 'foranias' ? <MapIcon size={28} /> : activeTab === 'parishes' ? <Church size={28} /> : <User size={28} />}
                      </div>
                      <div className="space-y-3">
                        <div>
                          ...
                          <h4 className="text-xl font-black text-[#131b2e] group-hover:text-blue-700 transition-colors">{item.name}</h4>
                          {activeTab === 'foranias' ? (
                            <p className="text-sm font-bold text-slate-500 mt-1 flex items-center gap-2 italic">
