    ...
    return (
      <div id="printable-student-record" className="hidden print:block text-black overflow-visible font-sans leading-tight">
        <div className="w-full">
          ...
          {/* RODAPÉ */}
          <div className="mt-auto border-t-2 border-black pt-2 flex justify-between items-start text-[7pt] font-bold text-black uppercase tracking-tight">
            <div className="flex-1 space-y-0.5">
              <p className="text-[8pt] font-black">{inst?.name || 'ESCOLA DIOCESANA DE MINISTÉRIOS'}</p>
              {inst?.address && <p>{inst.address} {inst.cep ? `- CEP: ${inst.cep}` : ''} {inst.city_uf ? `- ${inst.city_uf}` : ''}</p>}
            </div>
            <div className="text-right space-y-0.5">
              {inst?.phone && <p>TEL: {inst.phone}</p>}
              {inst?.email && <p>EMAIL: {inst.email}</p>}
            </div>
          </div>
        </div>
      </div>
